...
function toneShell(tone: HandoffGuidanceTone): string {
  switch (tone) {
    ...
    default:
      return "border-emerald-200/80 bg-gradient-to-br from-emerald-50/90 via-white to-white ring-1 ring-emerald-100/80";
  }
}

export function CustomerHandoffGuidanceCard({
  status,
  terminal,
  exactLocation,
  airportLabelText,
  ...
  const g = getCustomerHandoffGuidance(status, terminal);
  if (!g) return null;
  ...
  return (
    <section
      ...
      className={`mt-6 scroll-mt-28 rounded-2xl border p-5 sm:p-6 ${toneShell(g.tone)}`}
      aria-labelledby="customer-handoff-guidance-title"
    >
      {g.tone === "urgent" ? (
      ...
      <h2
        id="customer-handoff-guidance-title"
